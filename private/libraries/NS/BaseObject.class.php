...
class BaseObject
{
	 ...
	 */
	final protected function bindProperty($k, &$v)
	{
		$this->_var[$k]['v'] =& $v;
	}
	 ...
	 */
	final protected function bindProperties(&$p)
	{
		foreach ($p as $k => $v)
			$this->bindProperty($k, $p[$k]);
	}
	 ...
	 */
	final protected function createProperties($p, $nested = false)
	{
		foreach ($p as $k => $v)
			$this->createProperty($k, $v, $nested);
	}
	 ...
	 */
	final protected function setReadOnlyProperties($p)
	{
		foreach ($p as $k)
			$this->setReadOnlyProperty($k);
	}
	 ...
	 *@param array $p List of object property name
	 */
	final protected function setWriteOnlyProperties($p)
	{
		foreach ($p as $k)
			$this->setReadOnlyProperty($k);
	}
