...
class StyleManager extends SingletonObject
{
	...
	{
		$this->_styles[md5($style)] = $style;
	}
	function removeSource($location)
	{
		unset($this->_sources[md5($location)]);
	}
	function removeStyle($style)
	{
		unset($this->_styles[md5($style)]);
	}
	function getSources()
	{
	...
	function addExternalSource($location)
	{
		$this->_externalSources[md5($location)] = $location;
	}
	function removeExternalSource($location)
	{
		unset($this->_externalSources[md5($location)]);
	}
	...
	function renderTag($aggregator_path)
	{
		...
		foreach ($this->_externalSources as $s) {
	...
	function get($is_compressed = true)
	{
		...
		$contents = '';
		foreach ($this->_sources as $k => $source) {
			$contents .= file_get_contents($source);
			unset($this->_sources[$k]);
		}
		foreach ($this->_styles as $k => $style) {
			$contents .= $style;
