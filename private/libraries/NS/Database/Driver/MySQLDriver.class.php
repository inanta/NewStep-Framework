...
class MySQLDriver extends Database implements IDatabaseDriver {
	...
	*/
	function __construct($args = array()) {
		if(!function_exists('mysql_connect')) throw new LibraryException(array('code' => NS_EX_LIB_NOT_INSTALLED, 'class' => __CLASS__, 'library' => 'MySQL'));
	...
	*
	*/
	function affectedRows() { return @mysql_affected_rows($this->Connection); }

	/**
	*Close database connection to server
	*
	*/
	function close() { if(!$this->Persistent) @mysql_close($this->Connection); }
	...
	*/
	function connect() {
		...
		if(!is_resource($this->Connection)) {
			switch(mysql_errno()) {
				case 1045:
					throw new DatabaseException(array('code' => DatabaseException::UNABLE_TO_ACCESS, 'user' => $this->Username, 'database' => $this->Database)); break;
				case 2005:
				default:
	...
	*/
	function query($query) {
		...
		switch(@mysql_errno($this->Connection)) {
			case 1054:
				throw new DatabaseException(array('code' => DatabaseException::QUERY_COLUMN_NOT_EXIST, 'query' => $query)); break;
			case 1064:
				throw new DatabaseException(array('code' => DatabaseException::QUERY_ERROR, 'query' => $query)); break;
			case 1146:
				throw new DatabaseException(array('code' => DatabaseException::QUERY_TABLE_NOT_EXIST, 'query' => $query)); break;
			default:
